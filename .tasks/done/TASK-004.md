...
Agent: Codex on the Mac. Branch: task/04-ffmpeg. Depends: 001.
...
3. Operations (each returns the command as a String for logging and debug dumps):
   ...
   - `export_clip(path, start_s, end_s, out)`: `-ss S -to E -i IN -c copy OUT`; if `-c copy` produces a broken start (keyframe issue), fallback re-encode with `h264_videotoolbox` + AAC. Always try copy first and log which path was taken. (`libx264` from the draft is GPL-incompatible with this project's mandatory LGPL build.)
...
## Acceptance
- [x] `doctor` shows ffmpeg source and version; a release-layout simulation reports source = Bundled
- [x] Tests on fixtures: probe values match raw ffprobe JSON; sampled frame count ≈ duration×fps ±1; wav is 16 kHz mono; `frame_at(4.8)` matches the Task 3 PPM golden in dimensions and mean pixel within 2% (the committed golden is at the representative 40% time, not the stale draft's 1.0 s)
- [x] `export_clip` output plays and starts within 1 frame of requested start
- [x] Every command is logged verbatim at INFO and copied to `commands.txt` in debug mode
- [x] Progress callback fires during a 30 s export; cancelling an export leaves either a playable file or no file, never a corrupt one

## Do not
- Use ffmpeg bindings. Use PATH ffmpeg silently.

## Human review
- [x] Compared the 4.8 s JPEG thumbnail with the committed PPM golden; the timestamp and frame content match visually.
