...
Agent: Codex. Branch: task/09-search. Depends: 008 merged (for text embedding) — write against a trait so the search crate compiles without ort.
...
## Acceptance
- [ ] Unit: 10k random unit vectors, top-1 equals brute-force argmax; runtime < 30 ms
...
- [ ] Hybrid: a query word present only in a transcript lifts that shot above an otherwise-equal one
