...
| TASK-038 | Rename survival and shot-identity hardening (key feature) | OpenCode | backlog — task contract ready; found: `replace_shots` cleanup triggers destroy shot feedback/annotations/assessments/plan items on every resplit despite stable ids (data-loss class); open scope question for John: perform vs survive renames |
| TASK-039 | UX/UI enhancement track — full pass (craft + workflow) | Frontend lane | wave 1 in review — PR #42 (`task/39-ux-wave1`): collections reachable (P1), AA contrast, focus, Esc everywhere, grid-aware arrows, in-place search, reduced motion; reviewed, fixes applied; wave 2 (design tokens) next; John's call 2026-08-31: the first release DMG ships with the enhanced UI; audit in `docs/ux-enhancement-proposal.md` |
